...
function Assignment({ assignment, onClick, loading, setLoading }) {
  ...
  return (
    <div className="w-full py-1.5 min-h-64 sm:px-2 sm:w-1/2 md:w-1/3 lg:w-1/4 select-none">
      <Card
        className="w-full h-full overflow-hidden transition-all duration-150 relative cursor-pointer hover:brightness-98 dark:hover:brightness-85"
        onClick={handleClick}
      >
            ...
            <div className="flex gap-2 flex-row items-center">
              ...
              <p
                className={cn(
                  ...
                  new Date(due) < new Date() ? "text-failed" : "",
                )}
              >
                {formatDate(due)}
              </p>
