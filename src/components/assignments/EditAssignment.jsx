...
function EditAssignment({assignment, title=false, ...props}) {
  ...
  async function handleSubmit(payload, setError) {
    ...
    const result = await updateAssignment(assignment.id, payload, headers, setError);
    setOpen(false)
    return result;
  }
    ...
    <div onClick={(e)=>e.stopPropagation()}>
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="p-0 overflow-hidden h-fit max-h-[calc(9/10*100dvh)] gap-0 flex flex-col">
          ...
          <CreateAssignmentForm generated={assignment} submitButtonText="Edit" onSubmit={handleSubmit}/>
        </DialogContent>
      </Dialog>
    </div>
  );
