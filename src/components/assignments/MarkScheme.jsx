...
function MarkScheme({ mark_scheme }) {
  ...
  return (
    ...
    >
      ...
      <StyledCollapsible
        title={`Question ${mark_scheme[value].number}`}
        defaultOpen={true}
            ...
            <div key={i} className="flex flex-col gap-3">
              <div>
                ...
                <div className="border-2 p-2 rounded-sm mt-2">
                  <Truncate length={250}>{rubric.question}</Truncate>
                </div>
              </div>
              <div>
                ...
                <div className="border-2 p-2 rounded-sm mt-2">
                  <Truncate length={250}>{rubric.answer}</Truncate>
                </div>
              </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="rounded-sm mt-2 border-2 overflow-hidden">
                    {rubric.rubrics.map((rubric, i) => (
                      <div
                        key={i}
                        className="flex gap-4 justify-between border-1 p-2"
                      >
                        <span>{rubric.criterion}</span>
                        <span className="font-bold">{rubric.marks}</span>
                      </div>
                    ))}
                  </div>
                </div>
