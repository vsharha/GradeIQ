...
function EditSubmissionForm({assignment_id, submission, onSubmit}) {
  ...
  const {mutate, isPending} = useSubmissionMutation(assignment_id, async(payload) => {
    console.log(payload)
    const headers = await getClientAuthHeaders();
    const result = await updateSubmission(submission.id, payload, headers, form.setError);
